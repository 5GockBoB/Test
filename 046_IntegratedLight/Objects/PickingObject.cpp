...
bool PickingObject::Picking(vector<GameModel*>& objects)
{
	...
	for (GameModel* object : objects)
	{
		...
		D3DXMATRIX invWorld;
		D3DXMatrixInverse(&invWorld, NULL, &object->World());
		...
		object->GetCollider()->GetBoundingBox(boundingBox);
		...
		{
			float u, v, distance;
			if (D3DXIntersectTri(&boundingBox[indices[i * 3]], &boundingBox[indices[i * 3 + 1]], &boundingBox[indices[i * 3 + 2]], &start, &direction, &u, &v, &distance))
			{
