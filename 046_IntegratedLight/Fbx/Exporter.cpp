...
void Fbx::Exporter::GetClipList(vector<wstring>* list)
{
	...
	for (int i = 0; i < importer->GetAnimStackCount(); i++)
	{
		...
		list->push_back(String::ToWString(takeInfo->mName.Buffer()));
	}
}
...
string Fbx::Exporter::GetAnimName(UINT index)
{
	 return ReadAnimationData(index)->Name;
}
...
void Fbx::Exporter::ReadMaterial()
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		material->SpecularMapFile = Utility::GetTextureFile(prop);
...
void Fbx::Exporter::WriteMaterial(wstring saveFolder, wstring fileName)
{
	...
	for (FbxMaterial * material : materials)
	{
		...
		element = document->NewElement("SpecularMapFile");
		CopyTextureFile(material->SpecularMapFile, saveFolder);
		element->SetText(material->SpecularMapFile.c_str());
		node->LinkEndChild(element);
...
Fbx::FbxClip* Fbx::Exporter::ReadAnimationData(UINT index)
{
	...
	clip->Duration = (float)tempDuration.GetMilliSeconds();
	clip->FrameCount = (stop - start) + 1;
	
	return clip;
}

void Fbx::Exporter::ReadKeyframeData(FbxClip * clip, FbxNode * node, int start, int stop)
{
	FbxNodeAttribute* attribute = node->GetNodeAttribute();
