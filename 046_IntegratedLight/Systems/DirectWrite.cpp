...
void DirectWrite::RenderText(wstring text, RECT rect, float size, wstring font, D3DXCOLOR color, DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style, DWRITE_FONT_STRETCH stretch)
{
	...
	brushDesc.Color.r = color.r;
	brushDesc.Color.g = color.g;
	brushDesc.Color.b = color.b;
	brushDesc.Color.a = color.a;
	...
	if (findBrush == NULL)
	{
		deviceContext->CreateSolidColorBrush(brushDesc.Color, &brushDesc.Brush);
	...
	if (findText == NULL)
	{
		writeFactory->CreateTextFormat(textDesc.Font.c_str(), NULL, textDesc.Weight, textDesc.Style, textDesc.Stretch, textDesc.FontSize, L"ko", &textDesc.Format);
